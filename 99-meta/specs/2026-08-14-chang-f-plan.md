...
**F9 và F10 không phụ thuộc F4.** HC-5 có sẵn dữ liệu thật hôm nay (bốn lời gọi
...
| **F6** | OTel SDK + exporter + middleware tracing | `backend-erp/shared/middleware/tracing/`, `cmd/api/` | Span name là `c.FullPath()` (route pattern), **không** `c.Request.URL.Path`; `go.mod` chuyển otel từ `// indirect` sang phụ thuộc trực tiếp |
| **F7** | `TestMoiRouteV1SinhDungMotSpan` — test runtime cho HC-1 | `backend-erp/cmd/api/` | Dựng router thật + exporter trong bộ nhớ, lặp qua `r.Routes()`, bắn một request mỗi route dưới `/api/v1`, khẳng định **đúng một** span và `span.Name == route.Path`. **Không dùng đồng hồ** — exporter đồng bộ có điểm kết thúc gọi tên được (bài học chặng D) |
...
| **F9** | Checker HC-5 — đúng một chỗ sinh `request_id` | `backend-erp/arch/` | Ba dấu hiệu ở spec mục 4.1. **Bốn lời gọi `uuid.NewString()` đang có trong repo phải VẪN XANH** — đây là phép thử chứng minh checker phân biệt được |
...
3. **Phép thử phân biệt cho HC-5.** Thêm `requestID := uuid.NewString()` vào
   `modules/machine` → đỏ. Rồi xác nhận **bốn lời gọi `uuid.NewString()` đang có vẫn
...
6. **Phép thử ngược cho relay.** Tắt `cmd/relay` trong compose, tạo một sự kiện, xác nhận
   nó nằm lại `published_at IS NULL` — **và điều đó quan sát được trên `/metrics`** chứ
   không im lặng như hôm nay. Đây là thứ chặng này mua được mà không chặng nào trước có.
7. **Phép thử cho F12.** Làm hỏng cú pháp một file compose → CI của `infra-erp` đỏ. Hoàn
   lại.
8. `go run ./cmd/dev arch-update`, **đọc diff** — không dòng nào hạ mức, và ba dòng mới
   (HC-3, HC-4, HC-5) có `Unverifiable` khác rỗng.
