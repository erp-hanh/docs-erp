...
**Ngoại lệ:** Chỉ handler gọi `c.File(`, `c.FileAttachment(`, `c.DataFromReader(`, hoặc `c.Stream(` và không gọi `c.JSON(` trong cùng hàm mới được miễn; endpoint đó phải được liệt kê ở `04-conventions/C-API-http.md`. Cùng được miễn: handler bọc thẳng một `http.Handler` ngoài qua `gin.WrapH(` hoặc `gin.WrapF(`, và không gọi `c.JSON(` trong cùng hàm. Hình dạng này tồn tại vì có định dạng response do bên thứ ba định nghĩa mà bản thân nó không mang được envelope — ví dụ định dạng text của Prometheus: không client nào của nó đọc `{data, error, meta, request_id}`. Cái giá phải trả: bốn hình dạng trên mỗi cái chỉ khớp đúng một hàm cố định của `*gin.Context`, còn `gin.WrapH`/`gin.WrapF` bọc được bất kỳ `http.Handler` nào — đây là cửa rộng nhất trong năm, nên dòng đăng ký của nó ở `04-conventions/C-API-http.md` phải nêu rõ vì sao chính định dạng response đó không mang được envelope, không chỉ nêu tên endpoint.
**Principles:** P-ERR
**Decisions:** —
...
Cùng danh sách đóng đó, [ADR-0034](../03-decisions/ADR-0034-mot-tai-khoan-di-duoc-moi-phan-vung.md) mục 4 thêm hai tên: `PhanVungCuaToi` và `ChonPhanVung` của `AuthService` — hai đường của bước chọn phân vùng. Lý do miễn của chúng khác ba tên trên **một bậc**, và bậc đó phải được đọc ra chứ không gộp: ba method kia chạy khi chưa có actor nào, còn hai method này **có** actor — người gọi đã chứng minh mình là ai — nhưng actor ấy có thể đến từ một token danh tính, tức một actor **không mang phân vùng**. Vai trò thì tính theo cặp (người, phân vùng) theo [ADR-0019](../03-decisions/ADR-0019-phan-vung-la-cong-ty.md) mục 3, nên một lời gọi `authz.Can` ở đó hỏi một câu chưa có vế thứ hai, và câu trả lời luôn là không: chính người vừa đăng nhập thành công sẽ nhận `403` ở bước kế tiếp của luồng đăng nhập.

Cửa chặn của hai method ấy không nằm ở `authz.Can` mà nằm ở chính dữ liệu — cả hai đọc và ghi theo hàng `user_companies` của chính `actor.UserID`, trừ đúng một ngoại lệ mà [ADR-0036](../03-decisions/ADR-0036-quan-tri-he-thong-di-duoc-moi-phan-vung.md) mở ra cho cờ `users.is_system_admin`: người mang cờ đó đứng ngoài mọi phân vùng nên không có hàng `user_companies` nào để đọc theo, và cửa chặn của họ là chính cờ ấy, đọc lại từ database ở mỗi lần chọn. Ngoại lệ đó **không thêm tên nào** vào danh sách. Danh sách vẫn **đóng ở năm tên**: thêm tên thứ sáu phải sửa chính rule này, và nếu tên đó không thuộc `AuthService` thì việc phải làm là viết lại rule chứ không nối thêm một dòng.
**Principles:** —
**Decisions:** ADR-0009
...
**Dấu hiệu vi phạm:** Struct có field mà tên khớp `(?i)(password|passwd|matkhau|secret|token|apikey|api_key|privatekey|cccd|cmnd|otp)` nhưng tag không phải `json:"-"` (ví dụ vẫn còn `json:"password_hash"`). **Field kiểu `bool` được miễn khỏi dấu hiệu này** — một `bool` không mang được bí mật, nó chỉ lộ đúng một bit "có hay không", mà đó chính là thứ *Cách sửa* dưới đây đề nghị dùng. Không có ngoại lệ này thì rule bắt chính thứ nó vừa khuyên: `DaDatMatKhau bool json:"da_dat_mat_khau"` khớp regex. Lời gọi `logger.Info(`, `logger.Error(`, `fmt.Printf(`, `log.Println(` nhận một con trỏ hoặc một struct model làm tham số; logger chỉ được nhận cặp key-value kiểu nguyên thủy (string, số, bool).
